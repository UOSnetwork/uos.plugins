...
namespace eosio {
    ...
    public:
        ...
        void
        parse_transactions_from_block(const eosio::chain::signed_block_ptr &block, fc::mutable_variant_object &tags);
        ...
        void run_transaction(trx_to_run trx) {
            run_transaction(trx.account, trx.action, trx.data, trx.pub_key, trx.priv_key, trx.acc_from);
        }
...
//        if(current_head_block_number % 50 == 0) {
            ...
            elog("Staked balances snapshot for block " + to_string(current_head_block_number) + " ID :" +
                 current_head_id_block);
...
//                        elog("Handler not connected");
    ...
    void u_com_impl::run_trx_queue(uint64_t num) {
        if (num == 0)
            return;
        if ((fc::time_point::now() - app().get_plugin<chain_plugin>().chain().head_block_header().timestamp) <
            fc::seconds(3)) {
    ...
    void u_com_impl::run_transaction(transaction_queue &temp) {
        ...
        while (temp.size()) {
            act.data = eosio_token_serializer.variant_to_binary(temp.front().action, temp.front().data,
                                                                fc::milliseconds(100));
                    ...
                    [this](const fc::static_variant<fc::exception_ptr, chain::transaction_trace_ptr> &result) -> void {
                        if (result.contains<fc::exception_ptr>()) {
                            ...
                            try {
                                ...
                                pretty_output = app().get_plugin<chain_plugin>().chain().to_variant_with_abi(
                                        *trx_trace_ptr, fc::milliseconds(100));
    ...
    void u_com::set_program_options(options_description &, options_description &cfg) {
        cfg.add_options()
                ...
                ("start-block", boost::program_options::value<uint32_t>()->default_value(1),
                 "First block for parse transaction")
                ("end-block", boost::program_options::value<uint32_t>()->default_value(1),
                 "LAst block for parse transaction")
                ("parse-blocks", boost::program_options::value<bool>()->default_value(false),
                 "Parse all blocks (to current) at start plugin get transactions");
    }
    ...
    void u_com::irreversible_block_catcher(const eosio::chain::block_state_ptr &bst) {
        my->irreversible_block_catcher(bst);
    }
